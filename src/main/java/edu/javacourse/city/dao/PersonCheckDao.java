...
public class PersonCheckDao {
    ...
    private static final String SQL_REQUEST = "select temporal, upper(p.sur_name),upper('ермоленко' ) from cr_address_person ap " +
            "inner join cr_person p on p.person_id = ap.person_id " +
            "inner join cr_address a on a.address_id = ap.address_id " +
            "where " +
            "CURRENT_DATE >= ap.start_date and (CURRENT_DATE <= ap.end_date or ap.end_date is null)" +
            "and upper(p.sur_name) =  ? " +
            "and upper(p.given_name) =  ? " +
            "and upper(patronymic) =  ? " +
            "and p.date_of_birth = ? " +
            "and a.street_code = ?  " +
            "and upper(a.building) = ? ";
    ...
    public PersonResponse checkPerson(PersonRequest request) throws PersonCheckException {
        ...
        if (request.getExtension() != null) {
            sql += "and upper(a.extension) = ? ";
        } else {
            sql += " and extension is null ";
        }
        if (request.getApartment() != null) {
            sql += "and upper(a.apartment) = ? ";
        } else {
